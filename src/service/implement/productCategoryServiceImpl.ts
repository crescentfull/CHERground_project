...
export default class ProductCategoryServiceImpl implements ProductCategoryService {
    ...
    async deleteProductCategory(id: ProductCategoryDto, clearance: boolean): Promise<string> {
        if (clearance === true) {
            await this.productCategory.deleteProductCategory(Number(id.product), Number(id.category));
